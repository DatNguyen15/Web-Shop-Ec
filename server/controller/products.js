...
class Product {
  ...
  async postAddClicking(req, res) {
    ...
    if (!pId) {
      ...
      try {
        var product = await productModel.findById(pId);
        ...
        var userClick = await productModel.findOne({
          _id: pId,
          "pUserClick.user": uId,
        });
        ...
        await productModel.updateOne(
          ...
          {
            $inc: {
              "pUserClick.$.clicked": 1,
            },
            $set: {
              "pUserClick.$.lastClickedAt": Date.now(),
            },
          }
        );
  ...
  async getProductClick(req, res) {
    try {
      let Products = await productModel.find({}).limit(4).sort({ pclick: -1 });
  ...
  async getProductClickByUser(req, res) {
    ...
    try {
      let userByClick = await productModel
        .aggregate([{ $unwind: "$pUserClick" }])
        .sort({ "pUserClick.clicked": -1 });
      // .find({
      //   "pUserClick.user": uId,
      //  // "pUserClick.clicked": { $gt: 3 },
      // })
      // .limit(4)

      let productClick = userByClick
        .filter((val) => {
          return val.pUserClick.user == uId && val.pUserClick.clicked >= 3;
        })
        .slice(0, 4);
