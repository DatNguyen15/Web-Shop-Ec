...
class Category {
  ...
  async getDeleteCategory(req, res) {
    let { cId } = req.body;
    let haveProduct = await productModel.find({ pCategory: cId });
    console.log(haveProduct);
    if (!cId) {
      return res.json({ error: "All filled must be required" });
    } else if (haveProduct.length !== 0) {
      return res.json({
        error:
          "Category have products. You can edit category's status. Can't delete !!!",
      });
    } else {
      //return res.json({ error: "Can delete" });
      try {
