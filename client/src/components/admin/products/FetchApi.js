...
export const postClick = async (formData) => {
  try {
    let res = await axios.post(`${apiURL}/api/product/add-click`, formData);
    return res.data;
  } catch (error) {
    console.log(error);
  }
...
export const getAllProductSuggestion = async (uId) => {
  try {
    let res = await axios.get(`${apiURL}/api/product/get-click/${uId}`);
    return res.data;
  } catch (error) {
    console.log(error);
  }
