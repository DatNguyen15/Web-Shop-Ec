...
const ProductSuggestion = (props) => {
  ...
  const { productsugestion } = data;
  const history = useHistory();
  const clickProduct = (id) => {
    const formData = { pId: id };

    if (isAuthenticate()) {
      formData.uId = JSON.parse(localStorage.getItem("jwt")).user._id;
    }
  ...
  const fetchDataPro = async () => {
    ...
    try {
      let responseDataSuggestion = await getAllProductSuggestion(userId);
      console.log("product" + responseDataSuggestion);
      setTimeout(() => {
        if (responseDataSuggestion && responseDataSuggestion.Products) {
          dispatch({
            type: "setProductsClick",
            payload: responseDataSuggestion.Products,
          });
  ...
  return (
    <Fragment>
      ...
      {productsugestion && productsugestion.length > 0 ? (
        productsugestion.map((item, index) => {
          return item.pStatus === "Active" ? (
            <Fragment key={index}>
              <div className="relative col-span-1 m-2">
                <div className="card-product">
                  <div className="img-size">
                    <img
                      onClick={(e) => clickProduct(item._id)}
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <div className="p-3 text-gray-600 font-light truncate">
                      {item.pName}
                    </div>
